   ...
   $sql->db_Select("news_category", "*", "order by category_id", "no_where");
	...
	$newscategorylinks_text  .= $linkstyleset['linkstart'] ."<a href='" . e_HTTP . "' >" . LAN_THEME_7 ."</a>" .$linkstyleset['linkend'];
		
   while($row = $sql->db_Fetch() )
   {
	if ($pref['nclm_all'] == 0 ) {
		$newscategorylinks_text .= $linkstyleset['linkstart'] ."<a href='" . e_HTTP . "news.php?cat." . $row['category_id'] . "' >" . $row['category_name'] . "</a>". $linkstyleset['linkend'];
	} 
	else {
